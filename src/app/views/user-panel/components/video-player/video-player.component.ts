import { AfterContentInit, AfterViewChecked, AfterViewInit, Component, ElementRef, Input, OnChanges, OnDestroy, OnInit, Renderer2, Signal, SimpleChanges, ViewChild, inject, signal } from '@angular/core';
import $ from 'jquery';
import 'animate.css';
...
import {StreamType} from '../../../../shared/enum/streamtype.enum';
...
export class VideoPlayerComponent implements OnInit, OnChanges, OnDestroy, AfterViewInit,  AfterContentInit  {

  constructor(private renderer: Renderer2, private _http: HttpService, private _common:CommonService) {
  }
  ngAfterContentInit(){
  ...
  ngOnInit(): void {
    ...
    //   fetch('http://localhost:3001/stream/' + this.video.token + '/' + this.video.videoId)
    //     .then(response => {
    //       if (!response.ok) {
    ...
    //       this.renderer.setAttribute(vi, 'src', URL.createObjectURL(videoBlob));
    //     })
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    this.videoElement = document.querySelector(".main-video");
    this.loader = document.querySelector(".loader");
    if(this.loader){
    ...
    if (changes['vidData'].currentValue && this.vidData) {
      ...
      //Logic for complete streamining then play video.
      // this._http.getVideo('http://localhost:3001/stream/' + this.video.token + '/' + this.video.videoId).subscribe(
      
      // // this._http.getVideo('https://video-stream-server-q35d.onrender.com/stream/' + this.video.token + '/' + this.video.videoId).subscribe(
      //   (blob: Blob) => {
      //     this._common.isVideoPlayed.set(true);
      //     const videoUrl = URL.createObjectURL(blob);
      //     const vi = document.getElementById('customVideoPlayer');
...
this.players = document.querySelectorAll(".video_player");
// const selectedVid = document.querySelectorAll(".selectedVideo");
this.players.forEach((video_player) => {
  ...
  function playVideo() {
    ...
    playbtn.innerHTML = `<i class="fa fa-pause text-white m-0 p-0"></i>`;
    playbtn.classList.remove("animate__fadeIn");
    playbtn.classList.add("animate__fadeOut");
  ...
  function pauseVideo() {
    ...
    playbtn.innerHTML = `<i class="fa fa-play text-white m-0 p-0"></i>`;
    playbtn.classList.remove("animate__fadeOut");
    playbtn.classList.add("animate__fadeIn");
  ...
  for (let i = 0; i < mainVideoSources.length; i++) {
    let videoUrl = mainVideoSources[i].id;
    // blobUrl(mainVideoSources[i], 'http://localhost:3001/stream/'+videoUrl);
    // blobData('https://video-stream-server-38jb.onrender.com/stream/'+videoUrl);
    // if(videoUrl != 'null/null')
    // blobData('http://localhost:3001/stream/'+videoUrl);
  }

  function blobData(videoUrl) {
    const videoPlayer = document.getElementById("customVideoPlayer");
    // Fetch video stream from your Node.js API
    fetch(videoUrl)
      .then((response) => {
